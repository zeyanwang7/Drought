...
if ll1 == 1:
    ...
    out_q12 = muskingum_non_linear(Q12, 14, 0.29, 3, Q12[0], 1)
    out_q22 = muskingum_non_linear(Q22, 10, 0.35, 3, Q22[0], 1)
    out_q32 = muskingum_non_linear(Q32, 14, 0.29, 3, Q32[0], 1)
    out_q42 = muskingum_non_linear(Q42, 19, 0.37, 3, Q42[0], 1)
elif ll1 == 2:
    out_q11 = muskingum_non_linear(Q11, 10, 0.23, 3, Q11[0], 1)
    out_q21 = muskingum_non_linear(Q21, 8, 0.29, 3, Q21[0], 1)
    out_q31 = muskingum_non_linear(Q31, 10, 0.23, 3, Q31[0], 1)
    out_q41 = muskingum_non_linear(Q41, 15, 0.3, 3, Q41[0], 1)
    ...
    out_q11 = muskingum_non_linear(Q11, 8, 0.2, 3, Q11[0], 1)
    out_q21 = muskingum_non_linear(Q21, 6, 0.24, 3, Q21[0], 1)
    out_q31 = muskingum_non_linear(Q31, 8, 0.2, 3, Q31[0], 1)
    out_q41 = muskingum_non_linear(Q41, 10, 0.26, 3, Q41[0], 1)

    out_q12 = muskingum_non_linear(Q12, 8, 0.2, 3, Q12[0], 1)
    out_q22 = muskingum_non_linear(Q22, 6, 0.24, 3, Q22[0], 1)
    out_q32 = muskingum_non_linear(Q32, 8, 0.2, 3, Q32[0], 1)
    out_q42 = muskingum_non_linear(Q42, 10, 0.26, 3, Q42[0], 1)
...
for i in range(len(qzw1) - 1):
    w1 = w1 + (qzw1[i] + qzw1[i + 1]) * 3 * 60 * 30 / 1e8
    w2 = w2 + (qzw2[i] + qzw2[i + 1]) * 3 * 60 * 30 / 1e8
print("规程调度分洪量:", w1)
print("优化调度分洪量:", w2)

integers = list(range(1, len(q41_array) + 1))  # 生成1到Q长度的整数列表
x = integers
y1 = qzw1  # 第一条折线的纵坐标数据
y2 = qzw2  # 第二条折线的纵坐标数据
...
# 绘制第一条折线
if ll1 == 1:
    #ax1.plot(x, y1, 'r-', label='20年一遇洪水规程调度分洪流量')  # 'r-' 表示红色的实线
    ax1.plot(x, y2, 'r-', label='20年一遇洪水优化调度分洪流量')  # 'g-' 表示绿色的实线
elif ll1 == 2:
    #ax1.plot(x, y1, 'r-', label='50年一遇洪水规程调度分洪流量')  # 'r-' 表示红色的实线
    ax1.plot(x, y2, 'r-', label='50年一遇洪水优化调度分洪流量')  # 'g-' 表示绿色的实线
elif ll1 == 3:
    #ax1.plot(x, y1, 'r-', label='100年一遇洪水规程调度分洪流量')  # 'r-' 表示红色的实线
    ax1.plot(x, y2, 'r-', label='100年一遇洪水优化调度分洪流量')  # 'g-' 表示绿色的实线
