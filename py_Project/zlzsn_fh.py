...
ll = 2  # 优化 0 1 2 规程 1 2 3
l1 = 2 # 0 20年 1 50年 2 100年
# 提取第一列数据
...
w1 = 0
for i in range(len(qwa) - 1):
    w1 = w1 + (qwa[i] + qwa[i + 1]) * 3 * 60 * 30 / 1e8
print("规程调度分洪量:", w1)
...
# 绘制第一条折线
ax1.plot(x, y1, 'r-', label='分洪流量')  # 'r-' 表示红色的实线
...
fig.legend(loc='upper right', bbox_to_anchor=(0.31, 0.95))
