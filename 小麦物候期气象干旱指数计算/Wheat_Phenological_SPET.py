...
# 1. 基本参数设置
# =========================================================
spet_base_dir = r"E:\data\SPET\Wheat"
output_base_dir = r"E:\data\SPET\Wheat\Pheno_Stage_Mean"
...
# =========================================================
# 2. 工具函数：自动识别 SPET 变量名
# =========================================================
def detect_spet_var(ds):
    ...
    raise ValueError(f"无法自动识别 SPET 变量名，当前变量有: {candidates}")


# =========================================================
# 3. 工具函数：读取并标准化 SPET 数据
# =========================================================
def prepare_spet_dataset(spet_path):
    ds = xr.open_dataset(spet_path)

    print("=" * 100)
    print(f"📂 读取 SPET 文件: {spet_path}")
    print("=" * 100)
    print(ds)

    var_name = detect_spet_var(ds)
    print(f"\n✅ 自动识别 SPET 变量名: {var_name}")

    da = ds[var_name]

    if not {"time", "lat", "lon"}.issubset(set(da.dims)):
        raise ValueError(f"SPET 变量维度不是(time, lat, lon)体系，当前维度为: {da.dims}")
    ...
    print(f"\n📌 SPET 时间范围: {str(da.time.values[0])[:10]} ~ {str(da.time.values[-1])[:10]}")
    print(f"📌 SPET shape: {da.shape}")

    return ds, da, var_name


# =========================================================
# 4. 工具函数：读取 tif 并转为与 SPET 对齐的 DataArray
# =========================================================
...
def load_pheno_for_crop(crop, target_lat, target_lon):
    ...
    print(f"✅ {crop} 物候数据读取完成并已对齐到 SPET 网格")
...
# =========================================================
# 7. 核心函数：计算逐年两个阶段平均 SPET
# =========================================================
def calc_stage_mean_by_year(spet_da, gr_da, he_da, ma_da):
    # 物候有效掩膜
    valid_pheno = (
    ...
    # 每天的 DOY
    doy = xr.DataArray(
        spet_da.time.dt.dayofyear.values,
        coords={"time": spet_da.time},
        dims=("time",),
        name="doy"
    )
    ...
    # 仅保留阶段内 SPET
    spet_stage1 = spet_da.where(stage1_mask)
    spet_stage2 = spet_da.where(stage2_mask)

    # 按年求平均
    stage1_mean = spet_stage1.groupby("time.year").mean(dim="time", skipna=True)
    stage2_mean = spet_stage2.groupby("time.year").mean(dim="time", skipna=True)

    # 有效天数
    stage1_days = (stage1_mask & np.isfinite(spet_da)).groupby("time.year").sum(dim="time")
    stage2_days = (stage2_mask & np.isfinite(spet_da)).groupby("time.year").sum(dim="time")

    # 重命名和类型压缩
    stage1_mean = stage1_mean.rename("SPET_stage1_mean").astype(np.float32)
    stage2_mean = stage2_mean.rename("SPET_stage2_mean").astype(np.float32)
    stage1_days = stage1_days.rename("SPET_stage1_days").astype(np.int16)
    stage2_days = stage2_days.rename("SPET_stage2_days").astype(np.int16)
    ...
    out_ds = xr.Dataset({
        "SPET_stage1_mean": stage1_mean,
        "SPET_stage2_mean": stage2_mean,
        "SPET_stage1_days": stage1_days,
        "SPET_stage2_days": stage2_days,
        "GR_EM_DOY": gr_da,
        "HE_DOY": he_da,
        "MA_DOY": ma_da,
...
# 8. 工具函数：保存 nc
# =========================================================
def save_output_nc(ds_out, output_nc, crop, scale, spet_path, pheno_paths):
    encoding = {
        "SPET_stage1_mean": {"zlib": True, "complevel": 4, "dtype": "float32"},
        "SPET_stage2_mean": {"zlib": True, "complevel": 4, "dtype": "float32"},
        "SPET_stage1_days": {"zlib": True, "complevel": 4, "dtype": "int16"},
        "SPET_stage2_days": {"zlib": True, "complevel": 4, "dtype": "int16"},
    ...
    ds_out.attrs["title"] = f"{crop} SPET {scale}-month mean during two phenology stages"
    ds_out.attrs["crop"] = crop
    ds_out.attrs["spet_scale_month"] = scale
    ...
    ds_out.attrs["input_spet"] = spet_path
...
def quick_plot_check(ds_out, output_dir, crop, scale):
    ...
    s1 = ds_out["SPET_stage1_mean"].sel(year=year_pick)
    s2 = ds_out["SPET_stage2_mean"].sel(year=year_pick)

    fig1 = os.path.join(output_dir, f"{crop}_SPET_{scale}month_stage1_{year_pick}.png")
    fig2 = os.path.join(output_dir, f"{crop}_SPET_{scale}month_stage2_{year_pick}.png")

    plt.figure(figsize=(10, 5))
    s1.plot(cmap="RdBu", robust=True)
    plt.title(f"{crop} SPET-{scale}month Stage1 Mean ({year_pick})")
    plt.tight_layout()
    ...
    plt.title(f"{crop} SPET-{scale}month Stage2 Mean ({year_pick})")
    plt.tight_layout()
...
def print_output_summary(ds_out, crop, scale):
    print("\n" + "=" * 100)
    print(f"📊 输出结果摘要 | {crop} | SPET-{scale}month")
    print("=" * 100)
    print(ds_out)
    ...
    if "year" in ds_out.dims and len(ds_out["year"]) > 0:
        ...
        s1_mean = ds_out["SPET_stage1_mean"].sel(year=y0).mean(skipna=True).values
        s2_mean = ds_out["SPET_stage2_mean"].sel(year=y0).mean(skipna=True).values
        s1_days = ds_out["SPET_stage1_days"].sel(year=y0).mean(skipna=True).values
        s2_days = ds_out["SPET_stage2_days"].sel(year=y0).mean(skipna=True).values

        print(f"首年阶段1全区平均 SPET : {float(s1_mean):.4f}")
        print(f"首年阶段2全区平均 SPET : {float(s2_mean):.4f}")
...
def run_single_task(crop, scale):
    print("\n" + "#" * 120)
    print(f"🚀 开始任务: {crop} | SPET-{scale}month")
    print("#" * 120)

    spet_path = os.path.join(spet_base_dir, f"SPET_{scale}month_{crop}.nc")
    if not os.path.exists(spet_path):
        print(f"❌ SPET 文件不存在，跳过: {spet_path}")
        return

    # 读取 SPET
    _, spet_da, _ = prepare_spet_dataset(spet_path)

    # 读取物候
    gr_da, he_da, ma_da, pheno_paths = load_pheno_for_crop(
        crop,
        target_lat=spet_da.lat,
        target_lon=spet_da.lon
    )
    ...
    # 计算
    ds_out = calc_stage_mean_by_year(spet_da, gr_da, he_da, ma_da)

    # 输出路径
    out_dir = os.path.join(output_base_dir, crop)
    os.makedirs(out_dir, exist_ok=True)

    output_nc = os.path.join(out_dir, f"SPET_{scale}month_{crop}_PhenoStageMean_2002_2022.nc")

    # 保存
    save_output_nc(ds_out, output_nc, crop, scale, spet_path, pheno_paths)
...
if __name__ == "__main__":
    print("=" * 120)
    print("开始批量计算春冬小麦 1/3/6/9/12 个月 SPET 的两个物候阶段平均值（2002-2022）")
    print("=" * 120)
