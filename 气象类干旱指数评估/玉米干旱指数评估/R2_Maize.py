# -*- coding: utf-8 -*-
import os
import re
import numpy as np
import pandas as pd
from sklearn.linear_model import LinearRegression

# =========================================================
# 1. 文件路径
# =========================================================
stage1_file = r"E:\data\Province_Stage_Index_Mean\整理后\玉米阶段1干旱指数_宽表_2002_2022.xlsx"
stage2_file = r"E:\data\Province_Stage_Index_Mean\整理后\玉米阶段2干旱指数_宽表_2002_2022.xlsx"

# 玉米去趋势分离单产结果
yield_file = r"C:\Users\wangrui\Desktop\作物单产2001-2022\玉米去趋势分离单产结果_二次拟合.xlsx"

output_dir = r"E:\data\Province_Stage_Index_Mean\R2结果_气象单产_玉米"
os.makedirs(output_dir, exist_ok=True)

# =========================================================
# 2. 基础参数
# =========================================================
YEARS = list(range(2002, 2023))   # 2002-2022
MIN_VALID_SAMPLES = 6             # 至少6个有效年份才计算R2
YIELD_SHEET_NAME = "玉米_气象单产_二次"

MAIZE_SHEETS = ["Spring_Maize", "Summer_Maize"]

# =========================================================
# 3. 指定种植省份（你最新给定）
# =========================================================
SPRING_MAIZE_PROVINCES = [
    "河北", "山西", "云南", "四川", "陕西", "甘肃", "新疆", "宁夏",
    "内蒙古自治区", "辽宁省", "吉林", "黑龙江", "湖北", "湖南",
    "广西壮族自治区", "重庆市", "天津市", "北京市", "江西", "青海"
]

SUMMER_MAIZE_PROVINCES = [
    "山东", "河南", "河北", "山西", "云南", "四川", "陕西", "安徽",
    "甘肃", "贵州", "新疆", "江苏", "天津", "北京", "宁夏", "浙江"
]

# 统一成标准key，便于模糊匹配
CROP_PROVINCE_MAP_RAW = {
    "Spring_Maize": SPRING_MAIZE_PROVINCES,
    "Summer_Maize": SUMMER_MAIZE_PROVINCES
}

# =========================================================
# 4. 工具函数
# =========================================================
def normalize_province_name(name):
    """统一省份名称"""
    if pd.isna(name):
        return None

    name = str(name).strip()

    replace_dict = {
        "内蒙古自治区": "内蒙古",
        "广西壮族自治区": "广西",
        "宁夏回族自治区": "宁夏",
        "新疆维吾尔自治区": "新疆",
        "西藏自治区": "西藏",
        "北京市": "北京",
        "天津市": "天津",
        "上海市": "上海",
        "重庆市": "重庆",
        "辽宁省": "辽宁",
        "吉林省": "吉林",
        "黑龙江省": "黑龙江",
        "河北省": "河北",
        "山西省": "山西",
        "云南省": "云南",
        "四川省": "四川",
        "陕西省": "陕西",
        "甘肃省": "甘肃",
        "湖北省": "湖北",
        "湖南省": "湖南",
        "江西省": "江西",
        "青海省": "青海",
        "山东省": "山东",
        "河南省": "河南",
        "安徽省": "安徽",
        "贵州省": "贵州",
        "江苏省": "江苏",
        "浙江省": "浙江",
        "内蒙": "内蒙古",
    }

    name = replace_dict.get(name, name)

    suffixes = ["省", "市", "自治区", "壮族自治区", "回族自治区", "维吾尔自治区", "特别行政区"]
    for s in suffixes:
        if name.endswith(s):
            name = name.replace(s, "")

    return name.strip()


def province_key(name):
    """前两个字模糊匹配key"""
    name = normalize_province_name(name)
    if name is None or len(name) == 0:
        return None
    return name[:2]


# 构建每种作物允许的省份key集合
CROP_PROVINCE_KEY_MAP = {
    crop: set(province_key(p) for p in prov_list if province_key(p) is not None)
    for crop, prov_list in CROP_PROVINCE_MAP_RAW.items()
}


def parse_indicator_col(col_name):
    """
    解析指标列名，如：
    SPI_1month
    SPEI_3month
    SPET_12month
    SVPD_6month
    SEDI_9month
    """
    col_name = str(col_name).strip()
    m = re.match(r"^([A-Za-z]+)_(\d+month)$", col_name)
    if m:
        return m.group(1), m.group(2)
    return None, None


def load_yield_long_maize(yield_excel):
    """
    读取玉米气象单产表，转成长表：
    省份 | 省份key | 年份 | 单产
    """
    df = pd.read_excel(yield_excel, sheet_name=YIELD_SHEET_NAME)
    df = df.copy()
    df.columns = [str(c).strip() for c in df.columns]

    if "省份" not in df.columns:
        raise ValueError(f"{YIELD_SHEET_NAME} 缺少“省份”列")

    year_cols = [f"{y}年" for y in YEARS if f"{y}年" in df.columns]
    if len(year_cols) == 0:
        raise ValueError(f"{YIELD_SHEET_NAME} 未识别到年份列")

    df["省份"] = df["省份"].astype(str).str.strip()
    df["省份key"] = df["省份"].apply(province_key)

    df_long = df.melt(
        id_vars=["省份", "省份key"],
        value_vars=year_cols,
        var_name="年份",
        value_name="单产"
    )

    df_long["年份"] = df_long["年份"].str.replace("年", "", regex=False).astype(int)
    df_long["单产"] = pd.to_numeric(df_long["单产"], errors="coerce")

    df_long = df_long.dropna(subset=["省份key", "年份", "单产"]).reset_index(drop=True)
    return df_long


def load_stage_wide_maize(stage_excel):
    """
    读取玉米阶段宽表（每个文件有两个sheet：Spring_Maize, Summer_Maize）
    转成长表：
    年份 | 省份 | 省份key | 作物 | 指标列名 | 指数值 | 指数 | 尺度

    关键修改：
    对每个作物sheet，只保留该作物指定的种植省份
    """
    all_list = []

    xls = pd.ExcelFile(stage_excel)
    for crop_name in MAIZE_SHEETS:
        if crop_name not in xls.sheet_names:
            print(f"⚠ 未找到工作表: {crop_name}")
            continue

        df = pd.read_excel(stage_excel, sheet_name=crop_name)
        df.columns = [str(c).strip() for c in df.columns]

        required_cols = ["年份", "省份", "作物"]
        for c in required_cols:
            if c not in df.columns:
                raise ValueError(f"{stage_excel} - {crop_name} 缺少必要列: {c}")

        df["省份"] = df["省份"].astype(str).str.strip()
        df["省份key"] = df["省份"].apply(province_key)

        # -------------------------------------------------
        # 关键过滤：按指定种植省份过滤
        # -------------------------------------------------
        allowed_keys = CROP_PROVINCE_KEY_MAP.get(crop_name, set())
        before_n = len(df)

        df = df[df["省份key"].isin(allowed_keys)].copy()

        after_n = len(df)
        removed_n = before_n - after_n
        print(f"✅ {crop_name} 原始记录数: {before_n}，按指定省份过滤后: {after_n}，剔除: {removed_n}")

        if len(df) == 0:
            print(f"⚠ {crop_name} 过滤后无数据，跳过")
            continue

        meta_cols = {"年份", "省份", "作物", "阶段", "省份key"}
        indicator_cols = [c for c in df.columns if c not in meta_cols]

        valid_indicator_cols = []
        for c in indicator_cols:
            idx_name, scale_name = parse_indicator_col(c)
            if idx_name is not None:
                valid_indicator_cols.append(c)

        if len(valid_indicator_cols) == 0:
            raise ValueError(f"{stage_excel} - {crop_name} 没有识别到指标列")

        df_long = df.melt(
            id_vars=["年份", "省份", "省份key", "作物"],
            value_vars=valid_indicator_cols,
            var_name="指标列名",
            value_name="指数值"
        )

        df_long["年份"] = pd.to_numeric(df_long["年份"], errors="coerce").astype("Int64")
        df_long["指数值"] = pd.to_numeric(df_long["指数值"], errors="coerce")
        df_long = df_long.dropna(subset=["年份", "省份key"]).reset_index(drop=True)
        df_long["年份"] = df_long["年份"].astype(int)

        df_long[["指数", "尺度"]] = df_long["指标列名"].apply(
            lambda x: pd.Series(parse_indicator_col(x))
        )

        all_list.append(df_long)

    if len(all_list) == 0:
        raise ValueError(f"{stage_excel} 没有读到任何有效sheet")

    out = pd.concat(all_list, ignore_index=True)
    out = out.dropna(subset=["指数", "尺度"]).reset_index(drop=True)
    return out


def calc_r2_for_group(df_group):
    """
    对某个 省-作物-指标-尺度 计算R²
    回归形式：单产 ~ 干旱指数
    """
    sub = df_group[["指数值", "单产", "年份"]].dropna().copy()
    sub = sub[np.isfinite(sub["指数值"]) & np.isfinite(sub["单产"])]

    n = len(sub)
    if n < MIN_VALID_SAMPLES:
        return pd.Series({
            "有效年份数": n,
            "R2": np.nan,
            "回归系数": np.nan,
            "截距": np.nan
        })

    if sub["指数值"].nunique() < 2 or sub["单产"].nunique() < 2:
        return pd.Series({
            "有效年份数": n,
            "R2": np.nan,
            "回归系数": np.nan,
            "截距": np.nan
        })

    X = sub["指数值"].values.reshape(-1, 1)
    y = sub["单产"].values

    model = LinearRegression()
    model.fit(X, y)

    return pd.Series({
        "有效年份数": n,
        "R2": model.score(X, y),
        "回归系数": model.coef_[0],
        "截距": model.intercept_
    })


def compute_stage_r2_maize(stage_excel, yield_long_df, stage_name):
    """
    计算某一阶段R²
    春玉米 / 夏玉米干旱指数分开，
    但单产统一来自“玉米气象单产”
    """
    print("\n" + "=" * 90)
    print(f"开始计算 {stage_name} 的 R2")
    print("=" * 90)

    stage_long_df = load_stage_wide_maize(stage_excel)

    # 以单产表为标准：
    merged = pd.merge(
        stage_long_df,
        yield_long_df[["省份key", "年份", "单产"]],
        on=["省份key", "年份"],
        how="inner"
    )

    merged = merged.sort_values(["作物", "省份", "指数", "尺度", "年份"]).reset_index(drop=True)

    print(f"合并后记录数: {len(merged)}")
    if len(merged) > 0:
        print(f"作物: {merged['作物'].dropna().unique().tolist()}")
        print(f"指数: {merged['指数'].dropna().unique().tolist()}")

    result = (
        merged.groupby(["作物", "省份key", "指数", "尺度"], dropna=False)
        .apply(calc_r2_for_group)
        .reset_index()
    )

    province_name_map = (
        merged.groupby("省份key")["省份"]
        .agg(lambda x: x.value_counts().index[0] if len(x.dropna()) > 0 else None)
        .reset_index()
    )

    result = pd.merge(result, province_name_map, on="省份key", how="left")

    result["阶段"] = stage_name
    result = result[[
        "阶段", "作物", "省份", "省份key", "指数", "尺度",
        "有效年份数", "R2", "回归系数", "截距"
    ]]

    result = result.sort_values(["作物", "省份", "指数", "尺度"]).reset_index(drop=True)
    return result, merged


def build_best_indicator_table(result_df):
    """每个省、每个作物、每个阶段，选R²最高的指标"""
    sub = result_df.dropna(subset=["R2"]).copy()
    if len(sub) == 0:
        return pd.DataFrame(columns=["阶段", "作物", "省份", "最优指标", "最优尺度", "最大R2"])

    idx = sub.groupby(["阶段", "作物", "省份"])["R2"].idxmax()
    best = sub.loc[idx, ["阶段", "作物", "省份", "指数", "尺度", "R2"]].copy()
    best = best.rename(columns={
        "指数": "最优指标",
        "尺度": "最优尺度",
        "R2": "最大R2"
    })
    best = best.sort_values(["阶段", "作物", "省份"]).reset_index(drop=True)
    return best


# =========================================================
# 5. 主程序
# =========================================================
def main():
    print("=" * 90)
    print("春夏玉米两阶段干旱指数 vs 玉米气象单产（二次拟合）R2 计算")
    print("=" * 90)

    print("\n指定春玉米省份：")
    print(sorted([normalize_province_name(x) for x in SPRING_MAIZE_PROVINCES]))

    print("\n指定夏玉米省份：")
    print(sorted([normalize_province_name(x) for x in SUMMER_MAIZE_PROVINCES]))

    # ---------- 读取统一玉米气象单产 ----------
    yield_long_df = load_yield_long_maize(yield_file)
    print(f"\n✅ 玉米气象单产长表读取完成，记录数: {len(yield_long_df)}")
    print(yield_long_df.head())

    # ---------- 阶段1 ----------
    result_stage1, merged_stage1 = compute_stage_r2_maize(stage1_file, yield_long_df, "阶段1")

    # ---------- 阶段2 ----------
    result_stage2, merged_stage2 = compute_stage_r2_maize(stage2_file, yield_long_df, "阶段2")

    # ---------- 合并 ----------
    result_all = pd.concat([result_stage1, result_stage2], ignore_index=True)
    result_all = result_all.sort_values(["阶段", "作物", "省份", "指数", "尺度"]).reset_index(drop=True)

    best_df = build_best_indicator_table(result_all)

    # =====================================================
    # 6. 保存结果
    # =====================================================
    out_stage1 = os.path.join(output_dir, "玉米_阶段1_R2结果_气象单产.xlsx")
    out_stage2 = os.path.join(output_dir, "玉米_阶段2_R2结果_气象单产.xlsx")
    out_all = os.path.join(output_dir, "玉米_两阶段R2汇总结果_气象单产.xlsx")

    with pd.ExcelWriter(out_stage1, engine="openpyxl") as writer:
        for crop in MAIZE_SHEETS:
            sub = result_stage1[result_stage1["作物"] == crop].copy()
            if len(sub) > 0:
                sub.to_excel(writer, sheet_name=crop[:31], index=False)

        best1 = best_df[best_df["阶段"] == "阶段1"].copy()
        best1.to_excel(writer, sheet_name="阶段1最优指标", index=False)

    with pd.ExcelWriter(out_stage2, engine="openpyxl") as writer:
        for crop in MAIZE_SHEETS:
            sub = result_stage2[result_stage2["作物"] == crop].copy()
            if len(sub) > 0:
                sub.to_excel(writer, sheet_name=crop[:31], index=False)

        best2 = best_df[best_df["阶段"] == "阶段2"].copy()
        best2.to_excel(writer, sheet_name="阶段2最优指标", index=False)

    with pd.ExcelWriter(out_all, engine="openpyxl") as writer:
        result_all.to_excel(writer, sheet_name="All_R2_Results", index=False)
        best_df.to_excel(writer, sheet_name="Best_Indicator_By_Province", index=False)

        for crop in MAIZE_SHEETS:
            sub = result_all[result_all["作物"] == crop].copy()
            if len(sub) > 0:
                sub.to_excel(writer, sheet_name=f"{crop}_R2"[:31], index=False)

    print("\n" + "=" * 90)
    print("🎉 全部完成！")
    print("=" * 90)
    print(f"阶段1结果: {out_stage1}")
    print(f"阶段2结果: {out_stage2}")
    print(f"总结果  : {out_all}")


if __name__ == "__main__":
    main()