...
# --- 1. 路径设置 ---
# 使用之前生成的 SVPD 省份均值表
svpd_path = r'E:\data\计算结果\Provinces_Cotton_SVPD_Steps_2000_2022.csv'
# 产量表路径
yield_path = r'E:\data\计算结果\表格\棉花二次项去趋势标准化产量(SYRS).xlsx'
...
try:
    df_yield = pd.read_excel(yield_path, sheet_name='棉花', index_col=0).transpose()
except Exception as e:
    print(f"读取产量表失败，请检查 Sheet 名称是否为 '棉花': {e}")
...
df_res.to_csv(os.path.join(output_dir, 'Cotton_SVPD_Yield_Correlation.csv'), index=False, encoding='utf-8-sig')
...
for prov in common_provinces:
    ...
    plt.title(f'{prov}省 棉花产量对全年生育期 SVPD 的敏感性分析', fontsize=14)
...
print(f"✅ 计算完成！")
print(f"📊 相关性汇总表: {os.path.join(output_dir, 'Cotton_SVPD_Yield_Correlation.csv')}")
